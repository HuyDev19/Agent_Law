...
1. **Tải lên & Xử lý PDF**: Dùng chức năng "Tải lên & Xử lý PDF" ở sidebar để chọn file PDF từ máy tính. Sau khi chọn, nhấn nút **"Xử lý & Nạp Dữ liệu"**. Hệ thống sẽ đọc, xử lý và index các file này. Quá trình này chỉ cần làm một lần cho mỗi bộ dữ liệu mới.
   - _Lưu ý: Bạn cũng có thể đặt file PDF trực tiếp vào thư mục `data/` trước khi chạy ứng dụng._
2. **Tự động kết nối**: Ở những lần chạy sau, hệ thống sẽ tự động kết nối lại dữ liệu đã index từ thư mục `database/`, không cần nạp lại.
3. **Tra cứu**: Nhập câu hỏi bằng tiếng Việt tự nhiên vào ô tìm kiếm và nhấn **"🔎 Trích xuất câu trả lời"**.
...
_⚖️ Hệ thống chỉ mang tính chất tham khảo. Mọi quyết định pháp lý quan trọng cần tham vấn luật sư chuyên nghiệp._
